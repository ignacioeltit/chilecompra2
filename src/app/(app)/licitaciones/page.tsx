    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-full flex items-center justify-between gap-4">
          <h1 className="text-xl font-bold text-gray-900 flex-shrink-0">Licitaciones</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-50 transition-colors"
            >
              <Download className="h-4 w-4" />
              {seleccionadas.size > 0 ? `Exportar (${seleccionadas.size})` : 'Exportar'}
            </button>
            <button
              onClick={handleDeleteSelected}
              disabled={seleccionadas.size === 0 || deleting}
              className="inline-flex items-center gap-2 px-3 py-2 border border-red-300 text-red-700 rounded-lg text-sm hover:bg-red-50 transition-colors"
            >
              <Trash className="h-4 w-4" />
              {deleting ? 'Eliminando...' : (seleccionadas.size > 0 ? `Eliminar (${seleccionadas.size})` : 'Eliminar')}
            </button>
            <Link
              ...
              <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  <th className="px-4 py-3 w-8">
                    <input
                      type="checkbox"
                      checked={seleccionadas.size === pagActual.length && pagActual.length > 0}
                ...
                {pagActual.map(l => (
                  <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 max-w-[260px]">
                      <Link href={`/licitaciones/${l.id}`} className="hover:text-blue-600 line-clamp-2 text-sm font-medium">
                        {l.nombre}
                      </Link>
                      {l.descripcion && (
                        <p className="text-xs text-gray-400 line-clamp-1 mt-0.5">{l.descripcion}</p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className={l.horas_restantes > 0 && l.horas_restantes <= 24 ? 'text-red-600 font-semibold' : 'text-gray-700'}>
            ...
            {paginas > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
                <span className="text-sm text-gray-500">
                  Pág. {pagina} de {paginas} — {filtradas.length} resultados
                </span>
